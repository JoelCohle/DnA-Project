...
def InsertEmployee():
    ...
    Age = int(currentDate.year) - int(year)
...
while True:
    ...
    try:
        con = pymysql.connect(host = "localhost",user = "joel",password = "Pineapple$69",database = "RESTAURANT",cursorclass = pymysql.cursors.DictCursor)
        tmp = sp.call('clear', shell=True)
        if (con.open):
